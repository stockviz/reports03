...
fileTracker <- data.frame(ID = 0, STRAT_FILE = "", STRAT_TITLE = "", ASSET_CLASS = "")
...
for(i in 1:nrow(activePms)){
  ...
  for(j in 2:ncol(strategyRets)){
    if(nrow(na.omit(strategyRets[,j])) < 12) next
    
    stratTname <- colnames(strategyRets)[j]
    stratFname <- gsub(' ', '_', stratTname)
    ...
    fileTracker <- rbind(fileTracker, c(metaId, stratFname, stratTname, sebiClass))
  }
}
